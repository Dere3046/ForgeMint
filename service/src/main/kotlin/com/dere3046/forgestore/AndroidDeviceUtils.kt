...
object AndroidDeviceUtils {
    ...
    val osVersion: Int
        get() =
            DeviceAttestationService.cachedData?.osVersion
    ...
    fun getAttestVersion(securityLevel: Int): Int {
        ...
        val cached = DeviceAttestationService.cachedData?.attestVersion
        val version = cached ?: attestVersionMap[Build.VERSION.SDK_INT] ?: 400
        val source =
            when {
                cached != null -> "cache"
        ...
        Logger.d("attestVersion=$version source=$source securityLevel=$securityLevel")
        return version
    }
    ...
    internal fun setProperty(name: String, value: String) {
        try {
            Logger.d("Setting system property '$name' to: $value")
            val process = Runtime.getRuntime().exec(arrayOf("resetprop", name, value))
            val exitCode = process.waitFor()
            if (exitCode != 0) {
                val errorOutput = process.errorStream.bufferedReader().readText()
                Logger.e("resetprop for '$name' failed with exit code $exitCode: $errorOutput")
            }
        } catch (e: Exception) {
            Logger.e("Failed to set '$name' property via resetprop", e)
        }
    }
    ...
    val bootKey: ByteArray by lazy {
        initializeBootProperty(
            ...
            attestationValueProvider = { DeviceAttestationService.cachedData?.verifiedBootKey },
            expectedSize = 32,
    ...
    val bootHash: ByteArray by lazy {
        initializeBootProperty(
            ...
            attestationValueProvider = { DeviceAttestationService.cachedData?.verifiedBootHash },
            expectedSize = 32,
    ...
    fun describeSources(uid: Int): String {
        val customPatchLevel = ConfigManager.getPatchLevelForUid(uid) != null
        val osVersionSource =
            if (DeviceAttestationService.cachedData?.osVersion != null) "cache" else "map"
        return "osVersion=$osVersion(src=$osVersionSource) " +
            ...
            "teeCacheData=${DeviceAttestationService.cachedData != null}"
    }

    fun setupBootKeyAndHash() {
        Logger.d("Triggering initialization of boot key and hash")
        bootKey
        bootHash
        Logger.d("Boot key and hash initialization complete")
    }
    ...
    private fun initializeBootProperty(
        ...
        getProperty(propertyName, expectedSize)?.let {
            Logger.d("Using $propertyName from system property: ${it.toHex()}")
        ...
        try {
            attestationValueProvider()?.let {
                Logger.d("Using $propertyName from TEE attestation: ${it.toHex()}")
            ...
            Logger.e("Failed to get $propertyName from attestation", e)
        }

        readFromFile(propertyName, expectedSize)?.let {
            Logger.d("Using $propertyName from persistent file: ${it.toHex()}")
        ...
        return generateRandomBytes(expectedSize).also {
            Logger.d("Using randomly generated $propertyName: ${it.toHex()}")
    ...
    private fun getProperty(name: String, expectedSize: Int): ByteArray? {
        ...
        return if (value.length == expectedSize * 2) hexToByteArray(value) else null
    }

    private fun setProperty(name: String, bytes: ByteArray) {
        setProperty(name, bytes.toHex())
    }

    private fun setBootProperty(name: String, bytes: ByteArray) {
        if (!BootStateManager.shouldSpoofBootProps()) {
            Logger.i("Skipping system property '$name' because boot prop spoofing is disabled")
            return
        }
    ...
    private val PERSIST_DIR = File("/data/adb/forge_store")
    ...
    private fun persistToFile(propertyName: String, bytes: ByteArray) {
        try {
            ...
            Logger.e("Failed to persist $propertyName to file", e)
        }
    }
    ...
    private fun readFromFile(propertyName: String, expectedSize: Int): ByteArray? {
        return try {
            ...
            Logger.e("Failed to read $propertyName from file", e)
            null
        }
    }
    ...
    private fun getRealDevicePatchLevelInt(component: String, isLong: Boolean): Int {
        DeviceAttestationService.cachedData?.let { data ->
            val value =
                when (component) {
    ...
    private fun getCustomPatchLevelFor(uid: Int, component: String, isLong: Boolean): Int? {
        val config = ConfigManager.getPatchLevelForUid(uid) ?: return null
        val value =
            when (component) {
    ...
    private fun resolveDateKeywords(value: String): String {
        ...
        if (value.contains("YYYY", ignoreCase = true) ||
    ...
    private fun parsePatchLevelValue(value: String, isLong: Boolean): Int? {
        ...
        return try {
            ...
            Logger.w("Could not parse patch level value: $value", e)
            null
        }
    }

    private fun String.toPatchLevelInt(isLong: Boolean): Int {
        return parsePatchLevelValue(this, isLong) ?: 20240401
    }
    ...
    private val vintfKeyMintVersion: VintfKeyMintVersion? by lazy {
        readVintfKeyMintVersion().also { version ->
            if (version != null) {
                ...
                Logger.d("No usable KeyMint VINTF declaration found; using attestation fallback")
            }
        }
    }
        ...
        VINTF_MANIFEST_DIRS.forEach { path ->
            ...
            val listed =
                runCatching {
                    ...
                    Logger.d("Unable to list VINTF dir $path: ${throwable.message}")
                    null
                }
    ...
    val moduleHash: ByteArray by lazy {
        DeviceAttestationService.cachedData?.moduleHash
            ?.also { Logger.d("module-hash source=cache hash=${it.toHex().take(8)}") }
            ?: supplementaryModuleHash()?.also {
                Logger.d("module-hash source=framework-api hash=${it.toHex().take(8)}")
            }
            ?: runCatching {
                ...
                val modules =
                    apexInfos.map { (packageName, versionCode) ->
                        ...
                        ModuleEntry(nameOctet.encoded, sequence.encoded)
                    }

                val sortedModules =
                    modules.sortedWith { m1, m2 -> compareByteArrays(m1.nameEncoded, m2.nameEncoded) }
            ...
            .onSuccess { Logger.d("module-hash source=rederive hash=${it.toHex().take(8)}") }
            .onFailure { Logger.d("module-hash source=zero hash=00000000") }
            .getOrElse {
