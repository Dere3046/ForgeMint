...
object BootStateManager {
    private const val CONFIG_PATH = "/data/adb/forge_store"
    ...
    fun apply() {
        ...
        when (mode) {
            BootPropsMode.DISABLE -> {
                Logger.i("BootStateManager: disabled by $BOOT_PROPS_MODE_FILE")
                return
            }
            BootPropsMode.AUTO -> {
                if (isOplusFamilyDevice()) {
                    Logger.w("BootStateManager: skipping boot-state prop spoofing on Oplus-family device in auto mode")
                    return
                }
            }
            BootPropsMode.FORCE -> {
                Logger.i("BootStateManager: force-enabled by $BOOT_PROPS_MODE_FILE")
            }
        }
        ...
        for ((name, target) in targets) {
            ...
            if (current.isEmpty()) {
                Logger.d("BootStateManager: $name absent on this device, skip")
                continue
            }
            if (current == target) {
    ...
    private fun readBootPropsMode(): BootPropsMode {
        ...
        val raw =
            runCatching { file.readText().trim().lowercase() }
                .getOrElse {
                    Logger.w("BootStateManager: failed to read ${file.absolutePath}", it)
